...
SHMstruct * accessSHM(int fd) {
    ...
    struct_size = sizeof(SHMstruct); //get the full size of the shared memory block
...
SHMstruct * initSHM(int fd, SHMstruct *data) {
    ...
    //copy the data
    shm->ticket = data->ticket;
    shm->isTaken = data->isTaken;
    shm->soldOut = data->soldOut;
...
void clearSHM(SHMstruct * shm)
{
    ...
    if(munmap(shm, sizeof(SHMstruct) == -1)) {
        perror("Failed to close shared memory block\n");
        exit(1);
    }
}
