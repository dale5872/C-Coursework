...
int inCriticalRegion(sem_t *sem) {
    int lockCode;

    //check semaphore
    lockCode = sem_trywait(sem);

    if(lockCode == -1) {
        return 0; //semaphore locked
    } else if(lockCode == 0) {
        //we have now locked the semaphore so need to unlock it again
        leaveCriticalRegion(sem);
        return 1; //semaphore unlocked
    } else {
